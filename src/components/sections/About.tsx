      ...
      <div className="container">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'clamp(3rem, 6vw, 7rem)', alignItems: 'start' }} className="about-grid">
          <div className="about-left-el" style={{ opacity: 0 }}>
            ...
            <p style={{ fontSize: 14.5, color: '#6B6860', lineHeight: 1.8, marginBottom: '1.25rem' }}>We are a collective of developers, designers, and strategists passionate about building digital products that make a real difference. Founded with the belief that every business deserves world-class digital craftsmanship.</p>
            <p style={{ fontSize: 14.5, color: '#6B6860', lineHeight: 1.8, marginBottom: '2rem' }}>Our multi-disciplinary team brings deep expertise across full-stack development, e-commerce, CMS platforms, and visual design. Every project gets fresh eyes and strategic thinking.</p>
            <div style={{ borderLeft: '3px solid #C4622D', paddingLeft: '1.25rem', paddingTop: '0.75rem', paddingBottom: '0.75rem', background: '#fff', borderRadius: '0 12px 12px 0' }}>
              <p style={{ fontSize: 14, color: '#6B6860', lineHeight: 1.7 }}><strong style={{ color: '#1A1916' }}>"Great digital products are not built in isolation."</strong> We collaborate closely with clients, treating every engagement as a true partnership.</p>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 1.5, border: '1px solid #E8E6E1', borderRadius: 20, overflow: 'hidden', background: '#E8E6E1' }}>
            {values.map((v) => (
              <div key={v.num} className="about-value" style={{ opacity: 0, display: 'flex', gap: '1.25rem', alignItems: 'flex-start', background: '#fff', padding: '1.75rem', transition: 'background 0.2s' }}
                ...
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: '#9B9891', letterSpacing: '0.08em', paddingTop: 4, flexShrink: 0, width: 28 }}>{v.num}</span>
                <div>
                  <h4 style={{ fontSize: 14.5, fontWeight: 700, marginBottom: 6, letterSpacing: '-0.01em' }}>{v.title}</h4>
                  <p style={{ fontSize: 13.5, color: '#6B6860', lineHeight: 1.65 }}>{v.desc}</p>
                </div>
              </div>
            ))}
          </div>
