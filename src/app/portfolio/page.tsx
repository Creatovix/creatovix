...
export default function PortfolioSection() {
  ...
  // Fetch from Sanity once on mount
  useEffect(() => {
    sanityClient
      .fetch<Project[]>(getAllProjectsQuery)
              ...
              {CATEGORIES.map((cat) => {
                ...
                return (
                  ...
                  >
                    {cat}
                    {!loading && (
                      <span className="ml-1.5 opacity-50" style={{ fontSize: 9 }}>{count}</span>
                    )}
                  </button>
                );
              })}
...
function ProjectCard({ project }: { project: Project }) {
  ...
  const IMAGE_HEIGHT_PCT = 500;
  const MAX_SCROLL_PCT   = ((IMAGE_HEIGHT_PCT - 100) / IMAGE_HEIGHT_PCT) * 100;
  const DURATION         = 3200;
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden">
        <img
          ...
          style={{ height: `${IMAGE_HEIGHT_PCT}%`, objectPosition: "top", transform: "translateY(0%)", willChange: "transform", display: "block" }}
        />
      </div>
      ...
      >
        <div
          ...
          style={{ background: project.color, bottom: 0, transform: "translateX(-50%) translateY(0%)" }}
        />
      </div>
    </div>
  );
