...
/* ─────────────────────────────────────────────
   Project Card
   - No text overlays at all
   - Image starts at top, scrolls to bottom on hover
   - Returns to top smoothly on mouse leave
───────────────────────────────────────────── */
function ProjectCard({ project }: { project: (typeof PROJECTS)[0] }) {
  ...
  const MAX_SCROLL_PCT = IMAGE_HEIGHT_PCT - 100; // = 400%
  ...
  useEffect(() => {
    ...
    const tick = (timestamp: number) => {
      ...
      // Scroll image from 0% → -400% to show full height
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(180deg,rgba(5,3,16,0.08) 0%,rgba(5,3,16,0.25) 100%)",
          opacity: hovered ? 0 : 1,
        }}
      />
